...
import { checkinFor, dueItems, editView, hub, progress, today } from "../domain.js";
...
composer.callbackQuery("dashboard:show", async (ctx) => { await ctx.answerCallbackQuery(); await editView(ctx, summary(ctx), keys); });
